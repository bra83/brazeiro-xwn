...
MORAL — 2d6 (${morale.roll.join('+')}) = ${morale.total} vs Moral ${morale.morale}: ${morale.failed?'FALHOU':'MANTEVE-SE'}. [WWN SRD 5.3.1]`;
  ...
  function queryRules(state, question) {
    ...
    else if (/\bmoral\b|morale/.test(q)) answer = 'Moral: quando as circunstâncias justificam, role 2d6; se o resultado for maior que o valor de Moral, o NPC falha e tenta fugir, render-se ou interromper a luta conforme a situação. [WWN SRD 5.3.1]';
    ...
    else if (/\bataque\b|\bacertar\b|\bac\b/.test(q)) answer = 'Ataque de personagem: 1d20 + bônus base de ataque + modificador do atributo da arma + perícia de combate relevante. Sem nível-0 na perícia apropriada, a penalidade é -2. Igualar ou superar a AC relevante acerta. [WWN SRD 2.4.5]';
