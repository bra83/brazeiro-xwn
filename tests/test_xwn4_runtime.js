...
s.player.weapon.twoHanded=false;s.player.weapon.ammo=7;s.player.weapon.maxAmmo=10;let charswn=E.exportCharacter(s),charswnDoc=JSON.parse(charswn);eq(charswnDoc.character.weapon.twoHanded,false);eq(charswnDoc.character.weapon.ammo,7);let cross=E.makeInitialState('WWN'),crossBlocked=false;try{E.importCharacter(cross,charswn)}catch(_){crossBlocked=true}ok(crossBlocked,'cross-system character import accepted');let s2=E.makeInitialState('SWN');E.importCharacter(s2,charswn);eq(s2.player.weapon.ammo,7);
...
pc.x=5;pc.y=5;target.x=6;target.y=5;target.weapon.kind='melee';pc.weapon.twoHanded=false;pc.weapon.kind='ranged';pc.weapon.normalRange=100;pc.weapon.maxRange=300;let close=E.swAttack(s,'player','e0');ok(close.ok);eq(close.penalty,-4,'one-handed ranged melee penalty missing');pc.weapon.twoHanded=true;let blocked=E.swAttack(s,'player','e0');ok(!blocked.ok&&blocked.reason==='arma_duas_maos_em_melee');pc.weapon.twoHanded=false;
target.weapon.kind='ranged';target.x=5;target.y=2;combat.board.cover=[];pc.x=5;pc.y=8;pc.skills.shoot=-1;let untrained=E.swAttack(s,'player','e0');eq(untrained.total,untrained.d20-1,'untrained attack math wrong');
