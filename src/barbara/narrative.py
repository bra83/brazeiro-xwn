...
class NarrativePolicy:
    ...
    def turn_plan(self,text,mechanical=False,importance='normal'):
        ...
        return {'mode':mode,'kind':kind,'world_advances':mode=='fiction','question_count':qcount,'meaningful_uncertainty':meaningful_uncertainty,'check_required':check_required,'failure_modes':list(self.FAILURE_MODES) if check_required else [],'essential_clue_protected':kind=='investigation','stop_condition':'first_meaningful_uncertainty' if check_required else 'meaningful_decision','procedure':procedure,'channels':channels,'target_chars':list(self.target_chars(importance))}
    ...
    def validate_response_coverage(self,user_text,narration):
        ...
        rendered=str(narration).strip()
        # Multiple explicit questions require enough distinct answer structure; prevents one-line collapse.
    ...
    def validate_scene_ending(self,narration,importance='normal'):
        ...
        tail=str(narration).strip()[-320:].lower()
        decision_markers=('o que você','o que faz','como reage','qual é sua','sua vez','decisão','escolha','what do you','how do you','your move','choose')
