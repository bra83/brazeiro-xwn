...
class MechanicsAuthority:
    _EXPLICIT_MECHANICAL=(
        r'\b(rolo|rolar|role|jogo|jogar)\s+(?:um\s+)?(?:dado|dados|teste|ataque|per[ií]cia)\b',
        r'\b(fa[cç]o|fazer)\s+(?:um\s+)?teste\b',r'\bquanto\s+dano\b',r'\bteste\s+de\b',
        r'\b(roll|make)\s+(?:a\s+)?(?:check|test|attack|save)\b',r'\bhow much damage\b')
    _OUTCOME_ASSERTIONS=(
        r'\bvoc[eê]\s+(acerta|erra|falha|consegue|vence|perde)\b',
        r'\b(o|seu)\s+ataque\s+(acerta|erra|atinge|falha)\b',
        r'\bteste\s+(passa|falha|teve sucesso)\b',
        r'\byou\s+(hit|miss|fail|succeed|win|lose)\b',
        r'\b(?:the|your)\s+attack\s+(hits|misses|fails)\b')
    ...
    def validate_resolution(self,resolution):
        ...
        allowed={'outcome','roll','total','target','margin','details','source'}
